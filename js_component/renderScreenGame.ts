...
let mov = JSON.parse(localStorage.getItem('moves') as string);
mov++;
localStorage.setItem("moves", JSON.stringify(mov));
				//	localStorage.setItem('moves',Number(mov++) )
...
function updateSeconds() {
	...
	(spendTime  as HTMLElement).textContent = `${minutes.toString().padStart(2, '0')}.${seconds
		.toString()
		.padStart(2, '0')}`
}
